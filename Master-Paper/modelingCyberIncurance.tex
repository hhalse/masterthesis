...
<<<<<<< HEAD
Insurance companies often give a quantum discount when a customer purchases multiple products. From convenience stores, we are used to the slogan "buy one get one for free", and insurers tend to follow the same marketing strategy.  
...
>>>>>>> f7970f71c18bc003e64ed81274a1842efe292701
\subsection{Analysis}
...
<<<<<<< HEAD
We apply the discount to our model.
=======
...
At degree zero there is no discount on the insurance link cost, and thus if Eq.(\ref{eq:conditionitoi}) from previous model, insured nodes will connect to other insured nodes.
=======
