...
\cite{contagion}. Both the clique and star have the possibility of amplifying or suppressing selection and drift. This is favorable, because if the insurer is able to ensure that the nodes have a certain security level, one can prevent viruses from spreading.
...
\begin{figure}[h]
\centering
  \includegraphics[width=0.55\linewidth]{../Figures/BonusGameInsuredClique.png}
...
When we changed the link insurance cost to $I_{l}=0.5$, the resulting networks change. Now we found that the insured nodes are willing to establish risky links to reach their maximum degree. One example of a resulting network can be seen in figure \ref{fig:bonusvolating:b}.   Here the price of anarchy is $1$, i.e. it has reached the socially optimal outcome.
...
When the cost is low, a clique is the optimal structure, and when the cost is high a star is the optimal structure. 
