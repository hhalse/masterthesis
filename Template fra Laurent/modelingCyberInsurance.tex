...
This means that two nodes will end up in a classic prisoner's dilemma\footnote{Prisoner's dilemma}, where the best response is actually worse than the social optimal. In this case it is trivial to see that the social optimal scenario is for both nodes to choose $IL$. The reason for why we end up in $\overline{I}L$ is logical, since we assume either of the players have information about what decision the other will make. Therefore they assume the worst case, that the other node chooses not to buy insurance. Else they could risk ending up in a case where they pay $I_{l}$ without receiving any other benefit. 
