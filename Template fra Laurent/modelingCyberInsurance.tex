...
There is little doubt that it is necessary for companies and people to establish connections to each other. Without collaboration between nodes everyone would stand on their own, and our world would be extremely ineffective. Established connections might represent companies out sourcing part of their manufacturing, or cooperative agreements in the development of new software products. The later represents a scenario where a company does not have sufficient resources to develop the product alone, and hence collaborates with other companies to create the product. Each new connection is made with the intention of increasing the utility for a company. However, each connection also comes with a risk, which often is difficult to measure. Here, an intuitive example is the spread of epidemic diseases, also node failures of a power grid and financial contagion such as the one back in 2008 was a result of cascading failures. Strategic network formation using cyber-insurance can be used to prevent such situation in addition to increase the overall payoff of participants in a clustered network.
...
Another aspect of the network formation, is whether the final network will be stable or not. Stable means that at the network is in a state where no nodes have incentive to add or delete a link. In our models we use the concept of pairwise stability. Formally, a graph is pairwise stable if:
 \begin{enumerate}
...
\end{enumerate} 
It is important that our models will create networks that fulfills the criteria listed above, and ends up as a stable network. Unless, the networks can change, and hence the insurer cannot calculate the risks properly.
