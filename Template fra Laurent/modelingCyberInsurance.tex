...
In this simulation, the cost of insuring a link where set so that it violated the equation \ref{eq:final condition}. The result can be seen in figure \ref{fig:SimulationViolating}, and as we concluded with earlier, the result is one clique of both insured and non-insured nodes when $I_{l}<\beta-r$. And when the insurance cost is $I_{l}>\beta$, then only non-insured nodes connect to each other, because the insurance cost per link cost more then the benefit, so the insured ones choose not to connect to any one. 
