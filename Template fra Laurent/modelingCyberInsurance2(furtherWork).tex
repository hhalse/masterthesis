...
Since the networks formed are similar to scale-free networks, we can calculate the probability of a node in the network having the degree $g$, see (eq \ref{eq:probabilitydegree}). Where $n$-represents the number of nodes in the network, $\gamma$ is the power law parameter, as described in Chapter \ref{chp:graphTheory}.
\begin{equation}
P(g)=ng^{-\gamma}
...
To ensure that the network will form a star, we have to find parameters such that, equation (eq \ref{eq:discountstar}) is satisfied, and the probability of a node having a critical degree has to be relative low. In a network with ten nodes, the probability that there exists a node with degree, five, in a network with ten nodes, is $p=10*5^{-2}=0.4$. This is when we assume that the network follow a power-law with $\gamma=2$. With the parameters from table \ref{tbl:discountstar}, these conditions are satisifed and the resulting network can be seen in figure \ref{fig:startennodes}.
...
\end{figure}
These results where confirmed by running the simulation several times, and also by running several simulations with $n \geq 10 \text{ and} n\leq 40$. In every simulation the probability of existing a node with a critical degree was set to areas between $0.2 - 0.4$. And every time it resulted in a star-topology.

We also confirmed the results when running 
